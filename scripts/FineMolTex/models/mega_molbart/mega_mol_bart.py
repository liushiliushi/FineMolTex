...
from FineMolTex.cuchemcommon.workflow import BaseGenerativeWorkflow, add_jitter
...
class MegaMolBART(BaseGenerativeWorkflow):
    ...
    def __init__(self,
        ...
        self.device = device  # Megatron arg loading seems to only work with GPU
    ...
    def smileslist2embedding_model_given(self, model, smiles_list):
        ...
        token_ids = torch.tensor(self.tokenizer.convert_tokens_to_ids(tokens['original_tokens'])).to(self.device).T
        pad_mask = torch.tensor(tokens['masked_pad_masks']).bool().to(self.device).T
        token_ids = token_ids[:self.max_model_position_embeddings]
        pad_mask = pad_mask[:self.max_model_position_embeddings]
        encode_input = {"encoder_input": token_ids, "encoder_pad_mask": pad_mask}
